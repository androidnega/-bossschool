...
class StoreResultRequest extends FormRequest
{
    ...
    /**
     * Admin/Proprietor may pick a non-current term/year for back-entry; the
     * other roles get the current academic year + current term automatically.
     */
    protected function prepareForValidation(): void
    {
        ...
        if (! $canOverride || $this->input('academic_year_id') === null) {
            $this->merge(['academic_year_id' => $ctx->currentYear()?->id]);
        }
        if (! $canOverride || $this->input('term_id') === null) {
            $this->merge(['term_id' => $ctx->currentTerm()?->id]);
        }
    }
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator): void {
            ...
            // Only block when a NON-trashed result already exists for the same
            // student+subject+year+term combo.
            $exists = Result::query()
