...
@section('content')
        ...
        @if($ticket->attachments->count())
            <div class="mt-2 text-xs">
                ...
                @foreach($ticket->attachments as $a)
                    <a class="text-primary hover:underline" href="{{ route('support.attachment.download', $a) }}">{{ $a->original_name }}</a>{{ !$loop->last ? ',' : '' }}
                @endforeach
            </div>
        @endif
    </div>
    ...
    @if($ticket->status !== 'closed')
        <form method="POST" action="{{ route('support.reply', $ticket) }}" enctype="multipart/form-data" class="mt-4 space-y-2 rounded-xl border border-slate-200 bg-white p-3 text-sm">
            @csrf
            <textarea name="body" rows="4" required class="w-full rounded-md border border-slate-300 px-2 py-1.5" placeholder="{{ __('Add a reply…') }}"></textarea>
            <input type="file" name="attachment" class="block w-full text-xs" />
