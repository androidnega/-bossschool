...
@section('content')
    ...
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-semibold text-primary">{{ __('Edit fee') }}</h1>
    </div>

    <div class="max-w-xl rounded-lg border border-gray-200 bg-page p-6">
        <form method="POST" action="{{ route('fees.update', $fee) }}" class="space-y-6">
            @csrf
            @method('PUT')
            @include('fees._fields', ['classes' => $classes, 'terms' => $terms, 'fee' => $fee])
